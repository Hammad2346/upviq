  ...
  async function handleFetchAndAnalyze() {
    ...
    try {
      const fetchRes = await axios.get(
        `/api/profile/fetch?user_id=${dbUser.id}`,
      );
    ...
    <div className="min-h-screen space-y-8 p-6">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-sm">
          Connect your Upwork account to pull your profile and run an AI
          analysis.
        </p>
      </div>
      ...
      {!upworkConnected ? (
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, var(--color-primary) / 0.15, var(--color-secondary) / 0.1)",
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-sm">
              We'll fetch your profile directly from Upwork so the analysis
              reflects your real, live data.
            </p>
          </div>
          <Button
            onClick={handleConnectUpwork}
            disabled={!dbUser?.id || authLoading}
            className="..."
          >
            Connect Upwork
          </Button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Ready to fetch your latest profile data
              </p>
            </div>
          </div>
          <Button
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <div className="flex-1 space-y-2.5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <Briefcase size={12} /> {activeProfile.jobsRelatedCount} jobs
              </span>
              <span className="text-xs text-muted-foreground/50">·</span>
              <span className="text-xs text-muted-foreground">
                Fetched{" "}
                {new Date(activeProfile.scrapedAt).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {activeProfile.jobSuccess && (
                <Badge
                  icon={<Star size={11} />}
                  label={`${activeProfile.jobSuccess}% Job Success`}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatBox
          label="Job Success"
          value={
            activeProfile.jobSuccess ? `${activeProfile.jobSuccess}%` : "—"
          }
          highlight
        />
        <StatBox
          label="Related Jobs"
          value={
            activeProfile.jobsRelatedCount
              ? `${activeProfile.jobsRelatedCount}`
              : "—"
          }
        />
      ...
      <div className="glass-card glow-border rounded-2xl p-6 space-y-4">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {activeProfile.skills.length} skills listed on profile
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="glass-card glow-border rounded-2xl p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">
          {descExpanded ? activeProfile.description : descPreview}
          {!descExpanded && "…"}
        </p>
        <button
